      ...
      <div className="p-4 sm:p-10 flex flex-col md:flex-row gap-10 md:px-36">
              ...
              <div key={index} className="border border-gray-300 bg-white mb-2 rounded">
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    openSections[index] ? 'max-h-96 opacity-100 py-3' : 'max-h-0 opacity-0'
                  }`}
                >
                  <ul className="list-disc md:pl-10 pl-4 pr-4 text-gray-600 border-t border-gray-300">
                    {chapter.chapterContent.map((lecture, i) => (
                      <li key={i} className="flex items-start gap-2 py-2">
                        <img src={lecture.completed ? assets.blue_tick_icon : assets.play_icon} alt="Play Icon" className="w-4 h-4 mt-1" />
        ...
        <div className="md:mt-10 w-full md:w-1/2">
          {playerData ? (
            ...
            <img src={courseData?.courseThumbnail || assets.placeholder_image} alt="Course Thumbnail" className="w-full" />
          )}
        </div>
      </div>
      <Footer />
