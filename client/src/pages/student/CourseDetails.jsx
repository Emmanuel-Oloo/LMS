  ...
  useEffect(() => {
    if (allCourses?.length > 0) {
      ...
      if (findCourse) {
        setCourseData({
          ...findCourse,
          courseRating: Array.isArray(findCourse.courseRating) ? findCourse.courseRating : [],
          enrolledStudents: Array.isArray(findCourse.enrolledStudents) ? findCourse.enrolledStudents : [],
        });
      }
    }
  }, [allCourses, id]);
  ...
  return courseData ? (
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left">
        ...
        <div className="max-w-xl z-10 text-gray-500">
          ...
          <div className="flex items-center space-x-2 pt-3 pb-1 text-sm">
            ...
            <p className="text-gray-500">
              {courseData.courseRating.length} {courseData.courseRating.length === 1 ? 'rating' : 'ratings'}
            </p>
          </div>
          ...
          <div className='pt-8 text-gray-800'>
            ...
            <div className='pt-5'>
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded-lg shadow-sm'>
                  <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none bg-gray-50' onClick={() => toggleSection(index)}>
                    <div className='flex items-center gap-2'>
                      <img src={assets.down_arrow_icon} alt="Expand" className={`transition-transform duration-300 ${openSections[index] ? 'rotate-180' : ''}`} />
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openSections[index] ? 'max-h-96 opacity-100 py-3' : 'max-h-0 opacity-0'}`}>
        ...
        <div className='max-w-course-card shadow-custom-card rounded-t
         md:rounded-none min-w-[300px] sm:min-w-[420px] overflow-hidden'>
